...
<div style="text-align: right"> [Download pdf of Intro. to Cloud Storage](Data/Intro_to_Cloud_Storage.pdf) </div>
<div style="text-align: right"> [Raw data on GitHub](https://github.com/AdeArogundade/EEB_603_Tutorial_Fall2023) </div>
...
setwd("~/Documents/EEB_603_Tutorial_Fall2023_2")
...
### Install Git and R studio {#Install_Git_and_R_Studio}
...
```{r SearchTablePackages, echo=FALSE, warning=FALSE, include=FALSE}
...
```{r Git.Lexicon, eval = T, echo=F, warning = FALSE, message=FALSE}
###~~~
...
**Note:** 
  ...
  **- To operate GitBash, we are going to be working in the "Terminal" tab in R studio and not the "Console" **
  
  **- You do not need to include the $ sign in your code. It just indicates that you are running this code in the "Terminal"**
...
There are other Git clients to choose from, click [here](https://git-scm.com/downloads/guis) to explore other clients. For this tutorial, let's use Visual Studio Code as our Git editor.
...
### Install Git Credentials Manager (Only for Mac Users)

Mac User need to install a credential manager that helps stores and authenticate your git credentials and passwords. 

For PC or Windows users, this manager is installed alongside your Git software installation that was done in this [section](#Install_Git_and_R_Studio).
...
You can check this [link](https://github.com/git-ecosystem/git-credential-manager/blob/release/docs/install.md) for other installation options if `Homebrew` installation doesn't work for you.
...
This list was adapted from Sven Buerki's tutorial [page](https://svenbuerki.github.io/Genomics-Bioinformatics/Tutorials.html#5_Linux_Commands_Cheat_Sheet)
...
* Copy the text below into the mini window that appears after running the `nano README.md`command.
  ...
  **Note: You cannot use the mouse to position your cursor in a terminal text edit. Rather, use the arrow/direction keys to navigate.**
...
* For Fall 2023 EEB 603 students, we have assigned everyone a collaborator, so check this [Google Document](https://docs.google.com/spreadsheets/d/1gUwbqJ4gbhhSY1DfsZ7HsoDLzOKB-57xAAC1_IJ3qmk/edit#gid=0) that is saved in our shared Drive to get the information of your collaborator. 
...
Note:
  This command creates a new branch and changes to the new branch automatically. This is slightly different from the way we created a branch on Day 1 of this tutorial.
...
* Notice that this script does not contain a return code which is very important for a function because we would not get any output from it. 
...
Now, if you go to your GitHub page, and click on your own repository which is the repository name with yourinitials (**EEB_603_GitHub_Tutorial_Day2.YourInitials**), you can now see that the branch created by your collaborator (**function_collaboratorinitials**) has been added.
...
$ cd /path/to/your/EEB_603_GitHub_Tutorial_Day2.YourInitials/
  ...
  Note: Make sure to edit the path in this code to your own.
```
...
#### Create a R Markdown document and name it *index.Rmd*
...
* Now, use the `nano` command to create the website yaml file

Visit this [link](https://ymlthis.r-lib.org/reference/yml_site_opts.html) to learn more about the YAML file.
...
* Copy the code below **(without the - - - syntax)** into the window that appears after running the 'nano' command
...
README files are a quick and simple way for other users to learn more about your project. Wikis on GitHub help you present in-depth information about your project in a useful way. Check this [link](https://docs.github.com/en/communities/documenting-your-project-with-wikis/about-wikis) for more information about README and wiki.

#### License your work

Licensing your work is very important. Check the class [website](https://svenbuerki.github.io/EEB603_Reproducible_Science/Chapters.html#58_Creative_Common_licences) for more information on license. You can also check this [GitHub page](https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/licensing-a-repository)
...
It is common for a GitHub hosted website to be flagged by Google as "phishing and deceptive sites". If this happens to your website, you can fix using the following steps.

1. Visit [Google Search Console](http://search.google.com/search-console/)

2. Add your website URL into the domain area

3. Register and verify your website using one of the options that you prefer. Personally, I prefer the option of `HTML file`.

4. Download the `HTML file` and upload it into your GitHub repository.

5. Wait for few hours to a day for confirmation and the website should be back on. 

6. If not, go to the `Overview` tab on Google Search Console of your website, and if you see a message that states that your site has been blocked due to suspected copying.

7. Go to the comment and select the option to send an email to Google and request the removal of the block and explain in the email/message that you website is for educational purposes only. You should receive an email from Google after a few days that restriction/ban has been removed.
